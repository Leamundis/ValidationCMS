...
class Model
{
    ...
    public function updateOne($title, $content, $hidden, $log)
    {
        $request = $this->dbConnec->prepare('UPDATE ' . $this->tableName . ' SET title="' . $title . '", content="' . $content . '", hidden="' . $hidden . '", log="' . $log . '"  WHERE title="' . $title . '"');
        $request->execute();
        return true;
    }
}
